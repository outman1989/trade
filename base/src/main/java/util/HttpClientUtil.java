...
import util.httpClient.QtHttpClient;
import util.httpClient.QtHttpResult;
...
public class HttpClientUtil {
    ...
    public static int time_out = 5000;// 超时时间
...
//            get.getParams().setIntParameter(HttpConnectionParams.SO_TIMEOUT, time_out);// 设置数据传输超时时间
            // 设置代理
            if(StringUtil.isNotEmpty(proxyIP)) {
            ...
            LogUtil.error("HttpClientUtil.get", e);
        } finally {
            if (null != httpEntity) {
                try {
...
//            post.getParams().setIntParameter(HttpConnectionParams.SO_TIMEOUT, time_out);// 设置数据传输超时时间
            ...
            LogUtil.error("HttpClientUtil.post", e);
        } finally {
            if (null != httpEntity) {
                try {
                    httpEntity.consumeContent();
	...
	public static String postStream(String url, String data) {
        ...
        try {
            ...
            QtHttpResult qhr = qt.post(url,data);
            res=qhr.html;
        } catch (Exception e) {
            LogUtil.error("postStream", e);
        }
        return res;
	}
...
//            delete.getParams().setIntParameter(HttpConnectionParams.SO_TIMEOUT, time_out);// 设置数据传输超时时间
            ...
            LogUtil.error("HttpClientUtil.delete", e);
        } finally {
            if (null != httpEntity) {
                try {
                    httpEntity.consumeContent();
...
//            put.getParams().setIntParameter(HttpConnectionParams.SO_TIMEOUT, time_out);// 设置数据传输超时时间
            ...
            LogUtil.error("HttpClientUtil.putStream", e);
        } finally {
            if (null != httpEntity) {
                try {
                    httpEntity.consumeContent();
